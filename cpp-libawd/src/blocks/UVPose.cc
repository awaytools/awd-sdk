...
UVPose::UVPose(std::string& name, TYPES::UINT32 duration, TYPES::UINT32 external_start_time) :
	AttrElementBase(), 
...
TYPES::state UVPose::validate_block_state()
{
	return TYPES::state::VALID;
}

TYPES::F64 *
UVPose::get_transform_by_idx(int indx)
{
	/*
	int cnt=0;
	AWD_joint_tf *cur;
...
void
UVPose::set_next_transform(TYPES::F64 *mtx)
{
	/*
	AWD_joint_tf *tf;
...
TYPES::UINT32
UVPose::calc_body_length(AWDFile* awd_file, BlockSettings * settings)
{
	/*if(!this->get_isValid())
		return 0;*/
	TYPES::UINT32 len;
...
result
UVPose::write_body(FileWriter * fileWriter, BlockSettings * settings,AWDFile* file)
{
	/*
