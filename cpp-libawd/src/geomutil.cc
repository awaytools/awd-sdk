...
AWDGeomUtil::AWDGeomUtil(bool split_by_mat, bool force_split, bool useUV, bool useSecUVs, bool useNormals, double normalThreshold, int joints_per_vertex)
{
				...
				while ((subGeo = itsub.next()) != NULL){
					if (subGeo->triCnt>0){
						...
						if (subGeogroup->include_suv) {
							...
							sub->add_stream(SUVS, AWD_FIELD_FLOAT32, su_str, v_idx*2);
						}
						if (this->joints_per_vertex > 0) {
